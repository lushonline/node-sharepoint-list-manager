...
class ListClient extends BaseClient {
  ...
  addItems(items) {
    // Write operation so confirm X-RequestDigest configured
    this._requireDigest2();

    const promises = [];

    _.forEach(items, (item) => {
      promises.push(this.addItem(item));
    });
  ...
  /**
   * Update all items to list
   *
   * @param {object[]} items The items to update, they must have ID
   * @returns {Promise} Promise object with array of Axios response object
   */
  updateItems(items) {
    // Write operation so confirm X-RequestDigest configured
   ...
   * @param {object} [itemFilter={Filter: odatafilter().eq('ID', item.ID).toString()}]
   *                  The ODATA Query to check if item already exists, teh default assumes
   *                  the ID is supplied
   * @returns {Promise} Promise object with Axios response object
   */
  async upsertItem(item, itemFilter) {
    // Write operation so confirm X-RequestDigest configured
    this._requireDigest();
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const queryFilter = itemFilter || { Filter: odatafilter().eq('ID', item.ID).toString() };
   ...
   * @param {string|string[]} lookupitems The field[s] on the item we try and match
   * @returns {Promise} Promise object with array of Axios response object
   */
  async upsertItems(items, lookupitems) {
    // Write operation so confirm X-RequestDigest configured
    this._requireDigest();
    ...
    _.forEach(items, (item) => {
      promises.push(this.upsertItem(item, { Filter: this._buildFilter(item, lookupArray) }));
    });
