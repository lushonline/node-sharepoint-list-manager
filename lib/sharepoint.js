 ...
 * @returns {Promise} Promise object which after 2 seconds returns the configuration value message
 */
const getItems = (options, list, queryFilter) => {
  const { url } = options.sharepoint;
 ...
 */
const getAllItems = async (options, list, sharepointquery) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      while (keepGoing) {
        let response = null;
        opts.logger.info(`Sharepoint Query: ${stringifySafe(sharepointquery)}`, loggingOptions);

        try {
          // eslint-disable-next-line no-await-in-loop
          response = await getItems(opts, list, query);
        } catch (err) {
